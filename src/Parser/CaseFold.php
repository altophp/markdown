...
/**
 * Generated Unicode full case-fold table, Unicode 16.0.0, from
 * CaseFolding.txt status rows C and F. Do not edit by hand; regenerate with
 * `php bin/generate-casefold.php`. Keys are source codepoints; values are the
 * folded replacement as UTF-8. Codepoints absent here fold to themselves.
 *
 * @internal
 *
...
final class CaseFold
{
    ...
    /**
     * Full Unicode case fold of a UTF-8 string, table-driven, no mbstring.
     * Malformed or truncated sequences pass through byte for byte.
     */
    public static function fold(string $input): string
    {
        ...
        while ($index < $length) {
            $start = $index;
            $lead = \ord($input[$index]);

            if ($lead < 0x80) {
                $codepoint = $lead;
                ++$index;
            } elseif ($lead >= 0xF0 && $index + 3 < $length) {
                $codepoint = (($lead & 0x07) << 18)
                    | ((\ord($input[$index + 1]) & 0x3F) << 12)
                    | ((\ord($input[$index + 2]) & 0x3F) << 6)
                    | (\ord($input[$index + 3]) & 0x3F);
                $index += 4;
            } elseif ($lead >= 0xE0 && $index + 2 < $length) {
                $codepoint = (($lead & 0x0F) << 12)
                    | ((\ord($input[$index + 1]) & 0x3F) << 6)
                    | (\ord($input[$index + 2]) & 0x3F);
                $index += 3;
            } elseif ($lead >= 0xC0 && $index + 1 < $length) {
                $codepoint = (($lead & 0x1F) << 6)
                    | (\ord($input[$index + 1]) & 0x3F);
                $index += 2;
            } else {
                $output .= $input[$index];
                ++$index;
            ...
            $output .= self::TABLE[$codepoint] ?? \substr($input, $start, $index - $start);
        }
