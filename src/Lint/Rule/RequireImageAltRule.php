 ...
 */
final class RequireImageAltRule implements InlineRule
{
    public function id(): string
    {
        return 'require-image-alt';
    }

    public function inline(InlineEvent $event, RuleContext $context): iterable
    {
        if ('image' !== $event->kind || !$event->model instanceof ParsedDocumentModel) {
            return;
        }

        $text = $event->model->inlinePlainText($event->blockId->ordinal, $event->id->inlineOrdinal);
        ...
        yield new LintProblem(
            $this->id(),
            'Image must have alternative text.',
            $event->range,
